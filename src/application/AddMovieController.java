...
public class AddMovieController {
	...
	@FXML
    public void addMovie(MouseEvent event) throws JSONException, IOException {
		...
		obj.put("director", txtDirector.getText());
		obj.put("actors", txtActor.getText());
		obj.put("relyear", txtDate.getValue());
		
		System.out.println(obj);
		
		HttpClient client = HttpClientBuilder.create().build();
		HttpPost request = new HttpPost("http://localhost:56023/StockServices.svc/Post_AddMovie");
		...
		HttpResponse response = client.execute(request);
			...
			History.addToHistory(LoginController.employee.getId(), "CREATED", "New Movie with code : [ " + txtPcode.getText() + " ]");
	}
